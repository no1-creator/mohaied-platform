...
.ka-modal{background:#fff;border-radius:18px;max-width:640px;width:100%;max-height:90vh;overflow-y:auto;padding:22px;direction:rtl;}
...
export default function AdminKycPage() {
  ...
  async function openDetail(id: string) {
    ...
    try {
      ...
      setErr(e?.message || 'فشل تحميل التفاصيل');
    }
  }
  ...
  async function review(status: 'APPROVED' | 'REJECTED') {
    ...
    try {
      ...
      setErr(e?.message || 'فشل حفظ المراجعة');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <AdminShell active="kyc" title="توثيق الهوية">
      <style>{KA_CSS}</style>
      ...
      {loading ? (
        ...
        <div className="ad-empty">مفيش طلبات في القسم ده.</div>
      ) : (
            ...
            <tbody>
              {items.map((it) => (
                <tr key={it.id}>
                  ...
                  <td>{ID_TYPE_LABELS[it.idType] || it.idType}</td>
                  <td>{it.fullNameOnId}</td>
                  <td>
                    <span className={`ad-badge ${BADGE[it.status] || 'muted'}`}>
                      {STATUS.find((s) => s.key === it.status)?.label ||
                        it.status}
                    </span>
                  </td>
        ...
        <div className="ka-overlay" onClick={() => setDetail(null)}>
          <div className="ka-modal" onClick={(e) => e.stopPropagation()}>
            <div className="ka-modal-head">
              <h3>طلب توثيق — {detail.user?.fullName || detail.userId}</h3>
            ...
            <div className="ka-info">
              ...
              <div>
                ...
                <b>{ID_TYPE_LABELS[detail.idType] || detail.idType}</b>
              </div>
              <div>
              ...
              <div>
                ...
                <b>
                  ...
                  >
                    {STATUS.find((s) => s.key === detail.status)?.label ||
                      detail.status}
                  </span>
                </b>
              </div>
            ...
            <div className="ka-images">
              {detail.frontImage && (
                <a href={detail.frontImage} target="_blank" rel="noreferrer">
                  ...
                  <img src={detail.frontImage} alt="وجه المستند" />
                  <span>وجه المستند</span>
                </a>
              )}
              {detail.backImage && (
                <a href={detail.backImage} target="_blank" rel="noreferrer">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={detail.backImage} alt="ظهر المستند" />
                  <span>ظهر المستند</span>
                </a>
              )}
              {detail.selfie && (
                <a href={detail.selfie} target="_blank" rel="noreferrer">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={detail.selfie} alt="صورة شخصية" />
            ...
            {detail.reviewNote && (
              <p className="ka-prevnote">ملاحظة سابقة: {detail.reviewNote}</p>
            )}
            ...
            {detail.status === 'PENDING' ? (
              <>
                <textarea
                  className="ka-note"
                  placeholder="ملاحظة (اختياري) — تظهر للمستخدم لو رفضت"
                  value={note}
              ...
              <p className="ka-done">تمت مراجعة الطلب بالفعل.</p>
            )}
          </div>
        </div>
      )}
